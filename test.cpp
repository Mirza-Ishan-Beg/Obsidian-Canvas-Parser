...
int main(){
    ...
    */
    ...
    for (const auto& [group_ptr, card_list] : group_of_cards) {
        std::cout << "Group: " << group_ptr->name << "\n";
        for (const Dimensions* card_ptr : card_list) {
            std::cout << "  Card inside: ";
            card_ptr->print_data();
        }
    }
    */
    ...
    for (const auto& [group_ptr, card_list] : group_of_cards) {
        ...
        for (const Dimensions* card_ptr : card_list) {
            ...
            std::unordered_set<std::string> checked_patt = {". [X] ", "- [X] ", ". [x] ", "- [x] "};
            ...
            std::cout << checks_count << " " << unchecks_count << std::endl;
            if(checks_count || unchecks_count){
                ...
                std::cout
                    ...
                    << group_name
                    << " | Tasks Completed: "
                    << total_check_uncheck_count[group_name].completed
                    << " | Tasks Incompleted: "
                    << total_check_uncheck_count[group_name].pending
                    << std::endl;
            }
            //std::cout << "  Card inside: ";
            std::cout << "\n===================\n" << card_ptr->text << "\n===================\n";
...
// 2. Define methods *after* main():
Dimensions::Dimensions(std::string _name, int _a, int _b, int _c, int _d, std::string _text)
    : name(_name), x(_a), y(_b), width(_c), height(_d), text(_text)  // initialize with sample values
{
